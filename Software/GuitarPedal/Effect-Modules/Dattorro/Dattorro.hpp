...
//
// Original implementation: ValleyRackFree / Plateau (PlateauNEVersio)
...
//
// This copy carries several sample-rate-handling fixes made downstream (in
// bkshepherd/DaisySeedProjects' AmpSim module) relative to the original
// PlateauNEVersio source -- notably that Dattorro1997Tank::maxSampleRate is
// now actually wired to the constructor argument instead of being clamped to
// its 32 kHz default. See Effect-Modules/Dattorro/README.md for the full
// provenance chain and what each fix changes.
...
class Dattorro1997Tank {
  public:
    Dattorro1997Tank(const float initMaxSampleRate = 32000.0, const float initMaxLfoDepth = 0.0,
...
class Dattorro {
  public:
    Dattorro(const float initMaxSampleRate = 32000.0, const float initMaxLfoDepth = 16.0,
              const float initMaxTimeScale = 1.0);
