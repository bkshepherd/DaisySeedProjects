...
Wave<float> hann([] (float phase) -> float { return 0.5 * (1 - cos(2 * PI * phase)); }); // see if moving this to sdram significantly slows things down, YES, fails to load if allocated to SDRAM
...
// Delay
#define MAX_DELAY_SPECTRAL_DELAY static_cast<size_t>(188 * 4.f ) // 4 second max delay (4 second spread plus 1 second predelay), delay called 188 times per second
...
static const ParameterMetaData s_metaData[s_paramCount] = {
    ...
    {name: "TimeMode", valueType: ParameterValueType::Binned, valueBinCount: 5, valueBinNames: s_timeMode, defaultValue: {.uint_value = 0}, knobMapping: 3, midiCCMapping: 17},
    {name: "FDBK Mode", valueType: ParameterValueType::Binned, valueBinCount: 5, valueBinNames: s_fdbkMode, defaultValue: {.uint_value = 0}, knobMapping: 4, midiCCMapping: 18},
...
SpectralDelayModule::SpectralDelayModule()
    : BaseEffectModule(), m_cachedEffectMagnitudeValue(1.0f) {
...
void SpectralDelayModule::ParameterChanged(int parameter_id)  // Somewhere here is causeing issues on start up, if I take them out it works, adding them in breaks, but it worked once???
{
    ...
    if (parameter_id == 1 || parameter_id == 3) {  // Time or Time Mode
        ...
        for(int i=0; i<delay_array_size; i++) {
            if (delay_time_mode == 0) {
                ...
                delay_array_real[i].delayTarget = delay_array_imag[i].delayTarget =  r * 4 * 188 * vdelay_time;  // random delay time for each bin up to 4 seconds, mod 
              
            } else if (delay_time_mode == 1) {
                delay_array_real[i].delayTarget = delay_array_imag[i].delayTarget = (sin((i*cycles/delay_array_size) * 2 * PI) +1.0) * vdelay_time *188*2;  // sin wave scaled from 0 up to 4 seconds
              
            } else if (delay_time_mode == 2) {
                delay_array_real[i].delayTarget = delay_array_imag[i].delayTarget = vdelay_time * 4 * 188 * i / delay_array_size;  // linear delay time increase from low to high freq, 0 to 4 seconds, mod determines steepness of slope
                
            } else if (delay_time_mode == 3) {
                delay_array_real[delay_array_size - 1 - i].delayTarget = delay_array_imag[delay_array_size - 1 - i].delayTarget = vdelay_time * 4 * 188 * i / delay_array_size;  // linear delay time decrease low to high
        ...
        float cycles = 4.0; // when fdbk mode is center for sine wave, this changes the frequency of the sine wave across frequency bins
        ...
        for(int i=0; i<delay_array_size; i++) {
            if (delay_fdbk_mode == 0) {
                ...
                delay_array_real[i].feedback = delay_array_imag[i].feedback = (sin((i*cycles/delay_array_size) * 2 * PI) + 1.0) * vdelay_fdbk;
                ...
                delay_array_real[delay_array_size - 1- i].feedback = delay_array_imag[delay_array_size - 1- i].feedback = vdelay_fdbk * i / delay_array_size;
