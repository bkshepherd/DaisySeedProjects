...
class Utils {
  public:
    ...
    static inline void Copy(float *source, float *dest, int len) { std::memcpy(dest, source, len * sizeof(float)); }
    ...
    template <typename T> static float DB2gain(T input) { return std::pow(10, input / 20.0); }
