...
MyMappedFloatValue::MyMappedFloatValue(float min, float max, float defaultValue, Mapping mapping, const char *unitStr,
                                       uint8_t numDecimals, bool forceSign, float coarseStepSize0to1_, float fineStepSize0to1_)
...
void MyMappedFloatValue::Set(float newValue) { value_ = std::max(min_, std::min(max_, newValue)); }
...
void MyMappedFloatValue::ResetToDefault() { value_ = default_; }
